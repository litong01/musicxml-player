...
export class VerovioRendererBase {
  ...
  protected _recalculate(timemap: TimeMapEntryFixed[], svgs: string[]) {
    // Initialize the Verovio state.
    assertIsDefined(this._container);
    assertIsDefined(this._options);
    this._events = timemap.map((e: TimeMapEntryFixed) => {
      return { ...e, measureEntry: 0, rectNotes: [], notesOn: [] };
    });
    this._measures = [];
    this._currentNotes = [];
    ...
    this._events.forEach((event, eventEntry) => {
      ...
      event.notesOn =
        eventEntry > 0
          ? structuredClone(this._events[eventEntry - 1].notesOn)
          : [];
      ...
      [...(event.on ?? []), ...(event.restsOn ?? [])].forEach((domid) => {
        ...
        note.addEventListener('click', () => {
          this.player?.moveTo(
            measure.measure,
            measure.timestamp,
            event.tstamp - measure.timestamp,
          );
        });
  ...
  protected _refresh() {
    ...
    this._events.forEach((event, eventEntry) => {
      ...
      if ('measureOn' in event) {
        const measure = document.getElementById(event.measureOn)!;
        const system = measure.closest('g.system')!;
        this._measures[event.measureEntry].rectMeasure =
          measure.getBoundingClientRect();
        this._measures[event.measureEntry].rectSystem =
      ...
      if (eventEntry === 0) {
        measure.rectMeasure.width -=
          event.rectNotes[0].left - measure.rectMeasure.left;
  ...
  protected _move(
    ...
    while (
      eventEntry < this._events.length - 1 &&
      this._events[eventEntry + 1].tstamp <= timestamp
    ) {
      eventEntry++;
    }
        ...
        if (this._options.horizontal) {
          if (!duration) {
            const element = document.getElementById(
              notesOn.length ? notesOn[0] : this._measures[index].measureId,
            );
